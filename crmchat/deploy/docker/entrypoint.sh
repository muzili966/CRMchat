...
# 支付渠道：凭据只从环境变量注入、不入库不进后台；密钥填 PEM 文件路径或去掉头尾的一行 base64，
# 留空的渠道不会出现在收银台上。
# 值必须加双引号：框架按 ini 普通模式解析 .env，base64 末尾的 = 不加引号会让整份文件
# 解析失败，连数据库配置一起丢，应用直接起不来
...
PAY_WECHAT_APPID = "${PAY_WECHAT_APPID:-}"
PAY_WECHAT_MCHID = "${PAY_WECHAT_MCHID:-}"
PAY_WECHAT_SERIAL_NO = "${PAY_WECHAT_SERIAL_NO:-}"
PAY_WECHAT_PRIVATE_KEY = "${PAY_WECHAT_PRIVATE_KEY:-}"
PAY_WECHAT_API_V3_KEY = "${PAY_WECHAT_API_V3_KEY:-}"
PAY_WECHAT_PLATFORM_PUBLIC_KEY = "${PAY_WECHAT_PLATFORM_PUBLIC_KEY:-}"
PAY_WECHAT_PLATFORM_SERIAL = "${PAY_WECHAT_PLATFORM_SERIAL:-}"
PAY_MANUAL_ACCOUNT = "${PAY_MANUAL_ACCOUNT:-}"
PAY_MANUAL_QRCODES = "${PAY_MANUAL_QRCODES:-}"
