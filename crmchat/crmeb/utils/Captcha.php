...
class Captcha
{
     ...
     */
    public function create(array $generator = null, bool $api = false)
    {
        if (!$generator) {
        ...
        ob_start();
        // 输出图像
        $img     = imagepng($this->im);
        $content = ob_get_clean();
        imagedestroy($this->im);

        if ($api) {
            CacheService::set('captcha_record_' . $generator['key'], $generator['value'], $this->expire);
        }
     ...
     * @return string
     */
    protected function processDataUrl(string $img, string $mime = null)
    {
        $mime = $mime ?: 'image/png';
     ...
     * @return bool
     */
    public function checkApi(string $code, string $key = null): bool
    {
        if (!CacheService::has('captcha_record_' . $key)) {
            return false;
        }
        ...
        if (!CacheService::has($name)) {
            return false;
        }

        if (!$key) {
            if (!($generator = CacheService::get($name))) {
                return false;
            }
            $key = $generator['key'] ?? '';
        }
