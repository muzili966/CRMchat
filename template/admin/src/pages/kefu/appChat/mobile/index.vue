  ...
  <div class="chat-box">
    ...
    <div class="chat-scroll-box">
      <vue-scroll :ops="ops" ref="scrollBox" @refresh-activate="handleActivate" @refresh-start="handleStart" @refresh-before-deactivate="handleBeforeDeactivate" @refresh-deactivate="handleDeactivate">
        ...
        <div id="chatBox" class="chat" ref="chat" style="padding: .3rem;">
    ...
    <div class="footer-box">
      <div class="words" v-if="userToken" @click="showWords">
        <Upload :show-upload-list="false" :action="fileUrl" :before-upload="beforeUpload" :data="uploadData" :headers="header" :multiple="true" :on-success="handleSuccess" :format="['jpg','jpeg','png','gif']" :on-format-error="handleFormatError" style="margin-top: 1px;display: inline-block">
...
export default {
  ...
  methods: {
    ...
    goAdminOrder() {
      ...
      // })
      let url = window.location.protocol + "//" + window.location.host + '/pages/users/order_details/index?order_id=' + item.msn;
      window.open(url, '_blank');
    },
    // 滚动到底部
    height() {
